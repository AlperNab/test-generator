...
**Domain:** `Developer / QA` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Test Generator** is a standalone, browser-based AI workflow app for **Developer / QA**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
| `repo`<br>repo | text | Yes | — | Affects input: repo. |
...
| `coverage_target`<br>coverage target | text | No | — | Affects customization: coverage target. |
| `test_style`<br>test style | select | No | — | Affects customization: test style. |
| `mock_strategy`<br>mock strategy | text | No | — | Affects customization: mock strategy. |
| `ignored_files`<br>ignored files | text | No | — | Affects customization: ignored files. |
| `ci_provider`<br>CI provider | text | No | — | Affects customization: CI provider. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9159
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9159`. |
...
```bash
curl http://127.0.0.1:9159/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9159/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9159 uvicorn app.main:app --host 0.0.0.0 --port 9159
```
...
- `plugins/test-generator.json` for plugin metadata.
